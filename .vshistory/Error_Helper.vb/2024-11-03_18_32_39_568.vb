        ...
        Try
            Select Case BytesCaller
                Case Is >= 1099511627776
                    DoubleBytes = Error_VarConvert.ConvertToDouble(BytesCaller / 1099511627776) 'TB
                    Return FormatNumber(DoubleBytes, 0) & " TB"
                Case 1073741824 To 1099511627775
                    DoubleBytes = Error_VarConvert.ConvertToDouble(BytesCaller / 1073741824) 'GB
                    Return FormatNumber(DoubleBytes, 0) & " GB"
                Case 1048576 To 1073741823
                    DoubleBytes = Error_VarConvert.ConvertToDouble(BytesCaller / 1048576) 'MB
                    Return FormatNumber(DoubleBytes, 0) & " MB"
                Case 1024 To 1048575
                    DoubleBytes = Error_VarConvert.ConvertToDouble(BytesCaller / 1024) 'KB
...
#Region "Controls"
    ...
    Public Shared Sub ActivateControlsByLicense(ByVal ModulesList As List(Of ModuleDef))
        ...
        Try
            ...
            For Each sp As SettingsProperty In My.Settings.Properties
                ...
                If sp.Name.ToUpper.Contains("ALLOW") Then
                    For Each MnuModule As ModuleDef In ModulesList
                        Error_cCrypt.Encrypt(256, MnuModule.ModulName)
                        ModuleEntry1 = Error_cCrypt.EncryptedString
        ...
        Catch ex As Exception
            ErrorHandling.HandleErrorCatch(ex, GetCallingProc(), System.Reflection.MethodBase.GetCurrentMethod().Name, Environment.CurrentManagedThreadId, False, False)
            If IsIDE() Then Stop
        End Try
    End Sub
...
#Region "Dateien & Pfade"
    ...
    Public Shared Sub MoveFilesOlderThan(ByVal RootPath As String, ByVal OldPath As String, ByVal DaysKeepNew As Integer)
        ...
        Dim FSearch As New Error_FileSearch
        Dim Filter As New Error_FileSearch.SearchFilter
        Dim FileArray As New List(Of String)
        With Filter
            .Listing = Error_FileSearch.SearchFilter.LO.FILES_ONLY
            .FileTypes = "xml"
            .MaxDate = DateAdd(DateInterval.Day, DaysKeepNew * -1, Now)
            .NoSubFolders = True
            .DateType = Error_FileSearch.SearchFilter.DT.LAST_WRITE_TIME
        End With
    ...
    Public Shared Function DeleteFilesOlderThanDays(ByVal Path As String, ByVal FileAge As Integer, Optional ByVal Pattern As String = " * .* ") As Integer
        Try
            ...
            End If
            If FileCount > 0 Then
                Call Error_Logger.writelog(Level.Info, "Es wurden " & FileCount & " alte Dateien mit dem Pattern " & Pattern & " gelöscht.")
            End If
            Return FileCount
        Catch ex As Exception
            ErrorHandling.HandleErrorCatch(ex, Error_Helper.GetCallingProc(), System.Reflection.MethodBase.GetCurrentMethod().Name, Environment.CurrentManagedThreadId, False)
        ...
        Using mStream As MemoryStream = New MemoryStream()
            Using writer As XmlTextWriter = New XmlTextWriter(mStream, Encoding.Unicode)
                ...
                Catch unusedXmlException As XmlException
                    Call ErrorHandling.HandleErrorCatch(unusedXmlException, Error_Helper.GetCallingProc(), System.Reflection.MethodBase.GetCurrentMethod().Name, Environment.CurrentManagedThreadId, False, False)
                    If Error_Helper.IsIDE() Then Stop
                Catch ex As Exception
...
#Region "Datum & Zeit"
    Public Shared Function GetLastUpdateDate() As DateTime
        ...
        Try
            ...
            Dim TimeHour As Integer = Error_VarConvert.ConvertToInteger(VersionRevision.Substring(1, 2), 0)
            Dim TimeMinute As Integer = Error_VarConvert.ConvertToInteger(VersionRevision.Substring(3, 2), 0)
    ...
    Public Shared Function AddWorkingDays(ByVal DateIn As DateTime, ByVal ShiftDate As Integer) As DateTime
        ...
        While Weekday(datDate) = 1 OrElse Weekday(datDate) = 7 OrElse IsFeiertag
            datDate = datDate.AddDays(Error_VarConvert.ConvertToDouble(IIf(ShiftDate < 0, -1, 1)))
    ...
    Public Shared Function GetAlleFeiertage() As List(Of Error_FeierTage.TypeFeiertag)
        ...
        Dim M, N As Long
        Dim OsternA As Date
        Dim JahrA As Integer = Error_VarConvert.ConvertToInteger(Now.ToString("yyyy"), 0)
        Try
    ...
    Public Shared Function CalcHours(ByVal tvalue1 As ComboBoxEx, ByVal tvalue2 As ComboBoxEx, ByVal tvalue3 As ComboBoxEx, ByVal tvalue4 As ComboBoxEx) As String
        Try
            ...
            Dim minv1 As Integer = Error_VarConvert.ConvertToInteger(tvaluestr1(0), 0) * 60 + Error_VarConvert.ConvertToInteger(tvaluestr1(1), 0) 'convert2Minutes(tvalue1.Value)
            Dim minv2 As Integer = Error_VarConvert.ConvertToInteger(tvaluestr2(0), 0) * 60 + Error_VarConvert.ConvertToInteger(tvaluestr2(1), 0) 'convert2Minutes(tvalue2.Value)
            Dim minn1 As Integer = Error_VarConvert.ConvertToInteger(tvaluestr3(0), 0) * 60 + Error_VarConvert.ConvertToInteger(tvaluestr3(1), 0) 'convert2Minutes(tvalue3.Value)
            Dim minn2 As Integer = Error_VarConvert.ConvertToInteger(tvaluestr4(0), 0) * 60 + Error_VarConvert.ConvertToInteger(tvaluestr4(1), 0) 'convert2Minutes(tvalue4.Value)
    ...
    Public Shared Function CalcHours(ByVal tvalue1 As DateTimeInput, ByVal tvalue2 As DateTimeInput, ByVal tvalue3 As DateTimeInput, ByVal tvalue4 As DateTimeInput) As String
        ...
        Dim minv1 As Integer = Error_VarConvert.ConvertToInteger(tvaluestr1(0)) * 60 + Error_VarConvert.ConvertToInteger(tvaluestr1(1)) 'convert2Minutes(tvalue1.Value)
        Dim minv2 As Integer = Error_VarConvert.ConvertToInteger(tvaluestr2(0)) * 60 + Error_VarConvert.ConvertToInteger(tvaluestr2(1)) 'convert2Minutes(tvalue2.Value)
        Dim minn1 As Integer = Error_VarConvert.ConvertToInteger(tvaluestr3(0)) * 60 + Error_VarConvert.ConvertToInteger(tvaluestr3(1)) 'convert2Minutes(tvalue3.Value)
        Dim minn2 As Integer = Error_VarConvert.ConvertToInteger(tvaluestr4(0)) * 60 + Error_VarConvert.ConvertToInteger(tvaluestr4(1)) 'convert2Minutes(tvalue4.Value)
    ...
    Public Shared Function convert2Minutes(ByVal TimeStr As Date) As Integer
        ...
        End If
        Dim min As Integer = Error_VarConvert.ConvertToInteger(Split(tn1, ":")(0), 0) * 60 + Error_VarConvert.ConvertToInteger(Split(tn1, ":")(1), 0)
        Return min
    End Function
    ...
    Public Shared Function AddHours(ByVal TotalHours As String, ByVal Hours2Add As String) As String
        ...
        Dim int_TotalHours As Integer = Error_VarConvert.ConvertToInteger(tmpTotal(0), 0) * 60 + Error_VarConvert.ConvertToInteger(tmpTotal(1), 0)
        Dim tmpHours() As String = Hours2Add.Split(CType(":", Char))
        Dim int_Hours2Add As Integer = Error_VarConvert.ConvertToInteger(tmpHours(0), 0) * 60 + Error_VarConvert.ConvertToInteger(tmpHours(1), 0)
        Dim sum As Integer = int_TotalHours + int_Hours2Add
        Dim sumHours As Integer = Error_VarConvert.ConvertToInteger(Fix(sum / 60), 0)
        Dim sumMinutes As Integer = sum - (sumHours * 60)
        Return Error_VarConvert.ConvertToString(sumHours, False, "") & ":" & Error_VarConvert.ConvertToString(sumMinutes, False, "")
    End Function
    Public Shared Function HHmm2Minutes(ByVal TimeString As String) As Double
        ...
        If TimeString.Contains(":") Then
            ...
            hours = Math.Round(Error_VarConvert.ConvertToDouble(ts(0)) + (Error_VarConvert.ConvertToDouble(ts(1)) / 60), 2)
        End If
        Return hours
    ...
    Public Shared Function mmss2Seconds(ByVal TimeString As String) As Double
        ...
        If TimeString.Contains(":") Then
            Dim ts() As String = Split(TimeString, ":")
            minutes = Math.Round(Error_VarConvert.ConvertToDouble(ts(0)) + (Error_VarConvert.ConvertToDouble(ts(1)) / 60), 2)
        End If
        Return minutes
    ...
    Public Shared Function FormatTimeDez(ByVal nMin As Double) As Double
        ...
        If Application.CurrentCulture.Name = "de-DE" Then
            RetVal = Error_VarConvert.ConvertToDouble((nStd) & "," & (nMin1 / 60 * 100).ToString("00"))
        Else
            RetVal = Error_VarConvert.ConvertToDouble((nStd) & "." & (nMin1 / 60 * 100).ToString("00"))
        End If
        Return RetVal
    ...
    Public Shared Function String2Time(ByVal TimeString As String) As String
        ...
        If Not TimeString.Contains(":") AndAlso TimeString.Length >= 2 Then
            If timestr.Contains(":") Then
                ...
                hour = Error_VarConvert.ConvertToInteger(Mid(timestr, 1, ri - 1), 0)
                min = Error_VarConvert.ConvertToInteger(Mid(timestr, ri + 1, 1))
            ElseIf timestr.Length = 4 Then
                If IsNumeric(Mid(timestr, 1, 2)) AndAlso Error_VarConvert.ConvertToInteger(Mid(timestr, 1, 2)) <= 23 Then
                    hour = Error_VarConvert.ConvertToInteger(Mid(timestr, 1, 2))
                    min = Error_VarConvert.ConvertToInteger(Mid(timestr, 3, 2))
                ElseIf IsNumeric(Mid(timestr, 1, 1)) AndAlso IsNumeric(Mid(timestr, 2, 2)) Then
                    hour = Error_VarConvert.ConvertToInteger(Mid(timestr, 1, 1))
                    min = Error_VarConvert.ConvertToInteger(Mid(timestr, 2, 2))
                End If
            ElseIf timestr.Length = 3 Then
                If IsNumeric(Mid(timestr, 1, 2)) AndAlso Error_VarConvert.ConvertToInteger(Mid(timestr, 1, 2)) <= 23 Then
                    hour = Error_VarConvert.ConvertToInteger(Mid(timestr, 1, 2))
                    min = Error_VarConvert.ConvertToInteger(Mid(timestr, 3, 1))
                ElseIf IsNumeric(Mid(timestr, 1, 1)) AndAlso IsNumeric(Mid(timestr, 2, 2)) Then
                    hour = Error_VarConvert.ConvertToInteger(Mid(timestr, 1, 1))
                    min = Error_VarConvert.ConvertToInteger(Mid(timestr, 2, 2))
                End If
            ElseIf timestr.Length = 2 Then
                If IsNumeric(timestr) AndAlso Error_VarConvert.ConvertToInteger(timestr) <= 23 Then
                    hour = Error_VarConvert.ConvertToInteger(timestr)
                    min = 0
                ElseIf IsNumeric(Mid(timestr, 1, 1)) AndAlso IsNumeric(Mid(timestr, 2, 1)) Then
                    hour = Error_VarConvert.ConvertToInteger(Mid(timestr, 1, 1))
                    min = Error_VarConvert.ConvertToInteger(Mid(timestr, 2, 1))
                End If
            End If
    ...
    Public Shared Function SendSystemEmailUniversal(ByVal RegistryHiveValue As RegistryHive,
        ...
        Try
            If SMTPCred.SMTP_SenderAddress Is Nothing Then
                SMTPCred.SMTP_Password = DecryptString(Error_VarConvert.ConvertToString(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_Password")))
                SMTPCred.SMTP_User = Error_VarConvert.ConvertToString(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_User"))
                SMTPCred.SMTP_Server = Error_VarConvert.ConvertToString(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_RelayServer"))
                SMTPCred.SMTP_NoSSL = Error_VarConvert.ConvertToBoolean(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_NoSSL"))
                SMTPCred.SMTP_SSL = Error_VarConvert.ConvertToBoolean(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_SSL"))
                SMTPCred.SMTP_STARTTLS = Error_VarConvert.ConvertToBoolean(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_STARTTLS"))
                SMTPCred.SMTP_SenderAddress = Error_VarConvert.ConvertToString(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_SenderAddress"))
                SMTPCred.SMTP_SenderName = Error_VarConvert.ConvertToString(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_SenderName"))
                SMTPCred.SMTP_RecipientAddress = Error_VarConvert.ConvertToString(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_RecipientAddress"))
                SMTPCred.SMTP_RecipientName = Error_VarConvert.ConvertToString(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_RecipientName"))
                SMTPCred.SMTP_CC = Error_VarConvert.ConvertToString(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_CC"))
                SMTPCred.SMTP_BCC = Error_VarConvert.ConvertToString(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_BCC"))
                SMTPCred.SMTP_NoSSL_Port = Error_VarConvert.ConvertToInteger(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_NoSSL_Port"))
                SMTPCred.SMTP_SSL_Port = Error_VarConvert.ConvertToInteger(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_SSL_Port"))
                SMTPCred.SMTP_STARTTLS_Port = Error_VarConvert.ConvertToInteger(RegistryReadValue(RegistryHiveValue, RegistryPath & "\email", "SMTP_STARTTLS_Port"))
...
#Region "Fenster & Forms"
    Public Shared Sub CenterFormOnParent(ByVal ParentWindow As Form, ByVal Window As Form, ByVal RegistryHiveValue As RegistryHive, ByVal RegistryPath As String)
        ...
        Try
            Window_Location = Error_Helper.ReadWindowPosition(RegistryHiveValue, RegistryPath, ParentWindow, False, False, False)
    ...
    Public Shared Function ReadWindowPosition(ByVal RegHive As RegistryHive,
        ...
        Try
            For Each scr As Screen In Screen.AllScreens
                ...
                End If
            Next
            Window_X = Error_VarConvert.ConvertToInteger(RegistryReadValue(RegHive, RegistryPath & "\Parameter", "Location_" & WindowName & "_X"), 0)
            Window_Y = Error_VarConvert.ConvertToInteger(RegistryReadValue(RegHive, RegistryPath & "\Parameter", "Location_" & WindowName & "_Y"), 0)
            lp = New Point(Window_X, Window_Y)
            Window_Height = Error_VarConvert.ConvertToInteger(RegistryReadValue(RegHive, RegistryPath & "\Parameter", "Location_" & WindowName & "_Height", Error_Helper.VarTypeEnu.Type_Integer), 0)
            Window_Width = Error_VarConvert.ConvertToInteger(RegistryReadValue(RegHive, RegistryPath & "\Parameter", "Location_" & WindowName & "_Width", Error_Helper.VarTypeEnu.Type_Integer), 0)
    ...
    Public Shared Function fReadWindowPosition(ByVal RegistryHiveValue As RegistryHive,
        ...
        If WindowLocation Then
            Dim X As Integer = Error_VarConvert.ConvertToInteger(RegistryReadValue(RegistryHiveValue, RegistryPath & "\Parameter", "Location_" & WindowName & "_X"))
            Dim Y As Integer = Error_VarConvert.ConvertToInteger(RegistryReadValue(RegistryHiveValue, RegistryPath & "\Parameter", "Location_" & WindowName & "_Y"))
        ...
        If WindowHeight Then
            Dim WiHeight As Integer = Error_VarConvert.ConvertToInteger(RegistryReadValue(RegistryHiveValue, RegistryPath & "\Parameter", "Location_" & WindowName & "_Height", Error_Helper.VarTypeEnu.Type_Integer))
        ...
        End If
        If WindowWidth Then
            Dim WiWidth As Integer = Error_VarConvert.ConvertToInteger(RegistryReadValue(RegistryHiveValue, RegistryPath & "\Parameter", "Location_" & WindowName & "_Width", Error_Helper.VarTypeEnu.Type_Integer))
    ...
    Public Shared Sub WriteWindowPosition(ByVal RegHive As RegistryHive,
        ...
        If StartupComplete OrElse overwrite Then
            ...
            If WindowLocation Then
                rb = Error_Helper.RegistryWriteValue(RegHive, RegistryPath & "\Parameter", "Location_" & WindowName & "_X", Window.Location.X)
                rb = Error_Helper.RegistryWriteValue(RegHive, RegistryPath & "\Parameter", "Location_" & WindowName & "_Y", Window.Location.Y)
            End If
            If WindowHeight Then
                rb = Error_Helper.RegistryWriteValue(RegHive, RegistryPath & "\Parameter", "Location_" & WindowName & "_Height", Window.Height)
            End If
            If WindowWidth Then
                rb = Error_Helper.RegistryWriteValue(RegHive, RegistryPath & "\Parameter", "Location_" & WindowName & "_Width", Window.Width)
            End If
        End If
    End Sub
...
#Region "Images"
    ...
    Public Shared Function SafeImageFromFile(path As String) As Image
        ...
        If File.Exists(path) Then
            ...
            End Using
        End If
        Call Error_Logger.writelog(Level.Debug, "Die Grafik wurde als " & path & " gespeichert.")
        Return img
    End Function
    Public Shared Function ScaleImage(ByVal OldImage As Image, ByVal TargetHeight As Integer, ByVal TargetWidth As Integer) As Image
        ...
        End If
        Return New Bitmap(OldImage, Error_VarConvert.ConvertToInteger(NewWidth), Error_VarConvert.ConvertToInteger(NewHeight))
    End Function
    ...
    Public Shared Function GetImageFromByteArray(ByVal InputStream As FileStream) As Image
        ...
        Try
            Using BinaryReader As BinaryReader = New BinaryReader(InputStream)
                fileData = BinaryReader.ReadBytes(Error_VarConvert.ConvertToInteger(InputStream.Length))
            End Using
    ...
    Public Overloads Shared Function ResizeImage(bmSource As Bitmap, TargetWidth As Int32, TargetHeight As Int32) As Bitmap
        ...
        Using grDest As Graphics = Graphics.FromImage(bmDest)
            With grDest
                ...
                .DrawImage(CType(bmSource, Image), Error_VarConvert.ConvertToInteger(NewX), Error_VarConvert.ConvertToInteger(NewY), Error_VarConvert.ConvertToInteger(NewWidth), Error_VarConvert.ConvertToInteger(NewHeight))
            End With
        End Using
...
#Region "Konvertierungen"
    Public Shared Function GetDoubleFromString(ByVal Value As String) As Double
        ...
        If match.Success Then
            Return Error_VarConvert.ConvertToDouble(match.Value)
        Else
            Return 0
        End If
    End Function
...
#Region "Registry"
    ...
    Public Shared Function RegistryReadValue(ByVal Hive As RegistryHive,
                                         ...
                                         Key As String,
                                         ByVal Optional VarType As Error_Helper.VarTypeEnu = Error_Helper.VarTypeEnu.Type_String,
        ...
        Try
            ...
            ElseIf value Is Nothing AndAlso Not AllowNothing Then
                Select Case VarType
                    Case Error_Helper.VarTypeEnu.Type_Boolean
                        If DefaultValue Is Nothing Then
                            RetVal = False
                        Else
                            RetVal = CBool(DefaultValue)
                        End If
                    Case Error_Helper.VarTypeEnu.Type_Double
                        If DefaultValue Is Nothing Then
                            RetVal = 0
                        Else
                            RetVal = Error_VarConvert.ConvertToDouble(DefaultValue)
                        End If
                    Case Error_Helper.VarTypeEnu.Type_Integer
                        If DefaultValue Is Nothing Then
                            RetVal = 0
                        Else
                            RetVal = Error_VarConvert.ConvertToInteger(DefaultValue)
                        End If
                    Case Error_Helper.VarTypeEnu.Type_String
                        If DefaultValue Is Nothing Then
                            RetVal = ""
                        Else
                            RetVal = CStr(DefaultValue)
                        End If
                    Case Error_Helper.VarTypeEnu.Type_DateAsString
                        If DefaultValue Is Nothing Then
                            RetVal = ""
                        Else
                            RetVal = CStr(DefaultValue)
                        End If
                    Case Error_Helper.VarTypeEnu.Type_Date
                        If DefaultValue Is Nothing Then
            ...
            Else
                Select Case VarType
                    Case Error_Helper.VarTypeEnu.Type_Boolean
                        RetVal = CBool(value)
                    Case Error_Helper.VarTypeEnu.Type_Double
                        RetVal = Error_VarConvert.ConvertToDouble(Val(value))
                    Case Error_Helper.VarTypeEnu.Type_Integer
                        RetVal = Error_VarConvert.ConvertToInteger(Val(value))
                    Case Error_Helper.VarTypeEnu.Type_String
        ...
        Return RetVal
    End Function
    Public Shared Function RegistryReadValuePattern(Hive As RegistryHive, RegPathLocal As String, KeyPattern As String, Optional VariType As Error_Helper.VarTypeEnu = Error_Helper.VarTypeEnu.Type_String) As List(Of RegFindResult)
        Dim regKey As RegistryKey = Nothing
        Dim rd As Double = 0
        Dim rs As String = ""
    ...
    Public Shared Sub CopyRegistryFrom32BitTo64Bit(Hive As RegistryHive, RegPathLocal As String)
        Try
            Call Error_Logger.writelog(Level.Info, "Die 32-Bit Registry-Einträge werden nach 64-Bit kopiert.")
            Dim rb As Boolean = Error_Helper.RegistryWriteValue(Hive, RegPathLocal, "Test", "Test")
            ...
            rb = Error_Helper.RegistryWriteValue(Hive, RegPathLocal, "Test", "Test")
            Call RecurseCopyKey(SourceStartKey, DestinationStartKey)

            Call Error_Logger.writelog(Level.Info, "Die 32-Bit Registry-Einträge wurden erfolgreich nach 64-Bit kopiert.")
        Catch ex As Exception
            ErrorHandling.HandleErrorCatch(ex, Error_Helper.GetCallingProc(), System.Reflection.MethodBase.GetCurrentMethod().Name, Environment.CurrentManagedThreadId, False, False)
            If Error_Helper.IsIDE() Then Stop
        End Try
    ...
    Public Shared Sub CheckRegistryKey(ByVal RegHive As RegistryHive,
        ...
        Select Case VarType
            Case Error_Helper.VarTypeEnu.Type_Boolean
                If Error_Helper.RegistryReadValue(RegHive, RegPathLocal, Key, VarType, True) Is Nothing Then
            ...
            Case Error_Helper.VarTypeEnu.Type_DateAsString
                ...
                End If
            Case Error_Helper.VarTypeEnu.Type_Date
                Dim RetVal As Date = Error_VarConvert.ConvertToDate(Error_Helper.RegistryReadValue(RegHive, RegPathLocal, Key, VarType, True))
                'If RetVal Is Nothing Then
                '    Call Error_Helper.RegistryWriteValue(RegHive, RegPathLocal, Key, DefaultValue)
                'End If
            Case Else
                Dim RetVal As String = Error_VarConvert.ConvertToString(Error_Helper.RegistryReadValue(RegHive, RegPathLocal, Key, VarType, True), False, "")
...
#Region "System"
    ...
    Public Shared Function GetArchitecture() As String
        ...
        If Error_VarConvert.ConvertToInteger(IntPtr.Size) > 4 Then
            architecture = "64 Bit"
        Else
            architecture = "32 Bit"
        End If
...
#Region "Variablen"
    Public Shared Function Checkvar(ByVal var As Object,
        ...
        Try
            If VariType = Error_Helper.VarTypeEnu.Type_Boolean Then
                Return Error_VarConvert.ConvertToBoolean(var)
            ElseIf VariType = Error_Helper.VarTypeEnu.Type_Double Then

                Return Error_VarConvert.ConvertToDouble(var)
            ElseIf VariType = Error_Helper.VarTypeEnu.Type_Integer Then
                Return Error_VarConvert.ConvertToInteger(var)
            ElseIf VariType = Error_Helper.VarTypeEnu.Type_Long Then
                Return Error_VarConvert.ConvertToLong(var)
            ElseIf VariType = Error_Helper.VarTypeEnu.Type_Single Then
                Return Error_VarConvert.ConvertToSingle(var)
            ElseIf VariType = Error_Helper.VarTypeEnu.Type_String Then
            ...
            ElseIf VariType = Error_Helper.VarTypeEnu.Type_Date Then
            ...
            ElseIf VariType = Error_Helper.VarTypeEnu.Type_DateAsString Then
                If var Is Nothing OrElse System.Convert.IsDBNull(var) OrElse String.IsNullOrWhiteSpace(var.ToString) Then
    ...
    Public Shared Function Convert2Double(wert As String) As Double
        ...
        If IsNumeric(Val(wert)) Then
            Return Error_VarConvert.ConvertToDouble(Val(wert))
        Else
            Return 0
        End If
    End Function
    ...
    Public Shared Function IsDateWithinDays(ByVal CheckDate As String, Optional ByVal MaxDays As Integer = 30) As Boolean
        ...
        Dim ddiff As Integer = Error_VarConvert.ConvertToInteger(Math.Abs(DateDiff("d", CheckDate, Now)))
    ...
    Public Shared Function IncrementString(ByVal strString As String) As String
        ...
        For lngI = lngLenString To 0 Step -1
            ...
            strChar = Mid(strString, Error_VarConvert.ConvertToInteger(lngI), 1)
            If strChar = "z" Then
                ' If we find Z then increment this to A
                ' and increment the character after this (in next loop iteration)
                strString = Left$(strString, Error_VarConvert.ConvertToInteger(lngI - 1)) & "a" & Mid(strString, Error_VarConvert.ConvertToInteger(lngI + 1), Error_VarConvert.ConvertToInteger(lngLenString))
            Else
                ' Increment this non-Z and exit
                strString = Left$(strString, Error_VarConvert.ConvertToInteger(lngI - 1)) & Chr(Asc(strChar) + 1) & Mid(strString, Error_VarConvert.ConvertToInteger(lngI + 1), Error_VarConvert.ConvertToInteger(lngLenString))
                Exit For
            End If
        Next lngI
...
#Region "Unsortiert"
    Public Shared Function GetOrigin(ByVal PlatformSelect As Error_Helper.Platform) As String
        Select Case PlatformSelect
            Case Error_Helper.Platform.Afterbuy
                Return "Afterbuy"
            Case Error_Helper.Platform.Plenty
                Return "Plenty"
            Case Error_Helper.Platform.CSV
                Return "CSV"
            Case Error_Helper.Platform.eBay
                Return "eBay"
            Case Error_Helper.Platform.Amazon
                Return "Amazon"
            Case Error_Helper.Platform.Manuell
                Return "Manuell"
            Case Error_Helper.Platform.Shopware
                Return "Shopware"
            Case Error_Helper.Platform.Otto
                Return "Otto"
            Case Else
                Return "CSV"
        End Select
