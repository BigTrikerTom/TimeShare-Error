...
#Region "Properties"
    ...
    Public Shared Property SendCrashReport() As Boolean
        Get
    ...
    Public Shared Property SysMailRecipientAddress() As String
        Get
    ...
    Public Shared Property SysMailRecipientName() As String
        Get
    ...
    Public Shared Property SendAlwaysErrorEmail() As Boolean
        Get
...
#End Region
    ...
    Public Shared Sub GeneralErrorHandler(ByVal sender As Object, ByVal e As System.Threading.ThreadExceptionEventArgs)
        ErrorHandling.HandleErrorCatch(e.Exception, Error_Helper.GetCallingProc(), System.Reflection.MethodBase.GetCurrentMethod().Name, Environment.CurrentManagedThreadId, False, False)
    ...
    Public Shared Sub UnhandledErrorHandler(ByVal sender As Object, ByVal e As UnhandledExceptionEventArgs)
        ErrorHandling.HandleErrorCatch(CType(e.ExceptionObject, Exception), Error_Helper.GetCallingProc(), System.Reflection.MethodBase.GetCurrentMethod().Name, Environment.CurrentManagedThreadId, False, False)
        If Error_Helper.IsIDE() Then
            Stop
        Else
        End If
    ...
    Public Shared Function HandleErrorCatch(ByVal ex As Exception,
                                            ByVal parent_code_element_name As String,
                                            ByVal code_element_name As String,
                                            ByVal ThreadId As Integer,
                                            Optional ByVal ShowAlways As Boolean = False,
                                            Optional ByVal LogOnly As Boolean = False,
                                            Optional ByVal ZusatzInfo As String = "",
                                            Optional ByVal LogLevel As ErrorHandling.ErrorLogLevel = ErrorHandling.ErrorLogLevel.el_Error) As Boolean
        ...
        Dim errorText As String = "InnerException: " & errorInnerException & Environment.NewLine &
                                  ...
                                  "===================" & Environment.NewLine &
                                  "parent_code_element_name: " & parent_code_element_name & Environment.NewLine &
                                  "===================" & Environment.NewLine &
        ...
        Select Case LogLevel
            Case ErrorHandling.ErrorLogLevel.el_Debug
                pre = "Debug: "
            Case ErrorHandling.ErrorLogLevel.el_Error
                pre = "Fehler: "
            Case ErrorHandling.ErrorLogLevel.el_Fatal
                pre = "Fataler Fehler: "
            Case ErrorHandling.ErrorLogLevel.el_Info
                pre = "Info: "
            Case ErrorHandling.ErrorLogLevel.el_Warn
                pre = "Warnung: "
            Case Else
                pre = "Fehler: "
        End Select
        ...
        End If
        If ErrorHandling.check4InternetConnect() Then
        ...
        Select Case LogLevel
            Case ErrorHandling.ErrorLogLevel.el_Debug
                LogLevelText = "Debug"
            Case ErrorHandling.ErrorLogLevel.el_Error
                Call Error_Logger.writelog(Level.Error, sError, ex)
                LogLevelText = "Allgemeiner Fehler"
            Case ErrorHandling.ErrorLogLevel.el_Fatal
                Call Error_Logger.writelog(Level.Fatal, sError, ex)
                LogLevelText = "Fataler Fehler"
            Case ErrorHandling.ErrorLogLevel.el_Info
                Call Error_Logger.writelog(Level.Info, sError, ex)
                LogLevelText = "Info"
            Case ErrorHandling.ErrorLogLevel.el_Warn
                Call Error_Logger.writelog(Level.Warn, sError, ex)
                LogLevelText = "Warnung"
            Case Else
                Call Error_Logger.writelog(Level.Error, sError, ex)
                LogLevelText = "Allgemeiner Fehler"
        End Select
        If Error_Helper.IsIDE() OrElse ShowAlways Then
            If Not LogOnly Then
                ShowMsg(sError, Enumerations.ShowMsgImage.Alert, "Laufzeitfehler...", True, False, Sound.Houston_We_Have_A_Problem)
            End If
        End If

        Call ErrorHandling.SendErrorEmail(sError, LogLevelText)

        'If SendCrashReport AndAlso ErrorHandling.check4InternetConnect AndAlso Not Error_Helper.IsIDE Then
        '    SendReport(ex, sError)
        'End If

        Return True
    End Function
    Public Shared Function HandleErrorCatch(ByVal errorMessage As String,
                                            Optional ByVal parent_code_element_name As String = "",
                                            Optional ByVal code_element_name As String = "",
                                            Optional ByVal ThreadId As Integer = 0,
                                            Optional ByVal ShowAlways As Boolean = False,
                                            Optional ByVal LogOnly As Boolean = False,
                                            Optional ByVal ZusatzInfo As String = "",
                                            Optional ByVal LogLevel As ErrorHandling.ErrorLogLevel = ErrorHandling.ErrorLogLevel.el_Error) As Boolean
        ...
        sError = sError & "Message: " & Environment.NewLine
        sError = sError & Message
        If Not String.IsNullOrWhiteSpace(parent_code_element_name) Then
            sError = sError & "============================================" & Environment.NewLine
            sError = sError & "parent_code_element_name: " & parent_code_element_name & Environment.NewLine & Environment.NewLine
        End If
        If Not String.IsNullOrWhiteSpace(code_element_name) Then
            sError = sError & "============================================" & Environment.NewLine
            sError = sError & "code_element_name: " & code_element_name & Environment.NewLine & Environment.NewLine
        End If
        sError = sError & "============================================" & Environment.NewLine & Environment.NewLine
        Select Case LogLevel
            Case ErrorHandling.ErrorLogLevel.el_Debug
                LogLevelText = "Debug"
            Case ErrorHandling.ErrorLogLevel.el_Error
                Call Error_Logger.writelog(Level.Error, sError)
                LogLevelText = "Allgemeiner Fehler"
            Case ErrorHandling.ErrorLogLevel.el_Fatal
                Call Error_Logger.writelog(Level.Fatal, sError)
                LogLevelText = "Fataler Fehler"
            Case ErrorHandling.ErrorLogLevel.el_Info
                Call Error_Logger.writelog(Level.Info, sError)
                LogLevelText = "Info"
            Case ErrorHandling.ErrorLogLevel.el_Warn
                Call Error_Logger.writelog(Level.Warn, sError)
                LogLevelText = "Warnung"
            Case Else
                Call Error_Logger.writelog(Level.Error, sError)
                LogLevelText = "Allgemeiner Fehler"
        ...
        If Error_Helper.IsIDE() OrElse ShowAlways Then
            If Not LogOnly Then
                ...
                Catch ex As Exception
                    ...
                    If Error_Helper.IsIDE() Then Stop
                End Try
                ShowMsg(sError, ShowMsgImage.Alert, "Laufzeitfehler...", True, False, Sound.Houston_We_Have_A_Problem)
            End If
        End If
        Call ErrorHandling.SendErrorEmail(sError, LogLevelText)
    ...
    Private Shared Sub SendErrorEmail(ByVal ErrorMessage As String, ByVal LogLevel As String)
        ...
        Dim SMTPCred As New Error_Helper.SMTPCredentials
        Dim SMTP_RecipientAddress As String = SysMailRecipientAddress
        Dim SMTP_RecipientName As String = SysMailRecipientName
        If Not String.IsNullOrWhiteSpace(SMTP_RecipientAddress) AndAlso Not String.IsNullOrWhiteSpace(SMTP_RecipientAddress) AndAlso Not Error_Helper.IsIDE() OrElse SendAlwaysErrorEmail Then
            SMTPCred.SMTP_Password = Error_cCrypt.DecryptString(Error_VarConvert.ConvertToString(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_Password")))
            SMTPCred.SMTP_User = Error_VarConvert.ConvertToString(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_User"))
            SMTPCred.SMTP_Server = Error_VarConvert.ConvertToString(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_RelayServer"))
            SMTPCred.SMTP_NoSSL = Error_VarConvert.ConvertToBoolean(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_NoSSL"))
            SMTPCred.SMTP_SSL = Error_VarConvert.ConvertToBoolean(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_SSL"))
            SMTPCred.SMTP_STARTTLS = Error_VarConvert.ConvertToBoolean(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_STARTTLS"))
            SMTPCred.SMTP_SenderAddress = Error_VarConvert.ConvertToString(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_SenderAddress"))
            SMTPCred.SMTP_SenderName = Error_VarConvert.ConvertToString(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_SenderName"))
            SMTPCred.SMTP_RecipientAddress = Error_VarConvert.ConvertToString(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_RecipientAddress"))
            SMTPCred.SMTP_RecipientName = Error_VarConvert.ConvertToString(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_RecipientName"))
            SMTPCred.SMTP_CC = Error_VarConvert.ConvertToString(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_CC"))
            SMTPCred.SMTP_BCC = Error_VarConvert.ConvertToString(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_BCC"))
            SMTPCred.SMTP_NoSSL_Port = Error_VarConvert.ConvertToInteger(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_NoSSL_Port"))
            SMTPCred.SMTP_SSL_Port = Error_VarConvert.ConvertToInteger(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_SSL_Port"))
            SMTPCred.SMTP_STARTTLS_Port = Error_VarConvert.ConvertToInteger(Error_Helper.RegistryReadValue(Error_Helper.RegistryHiveValue, Error_Helper.RegistryPath & "\email", "SMTP_STARTTLS_Port"))
            ...
            If Not String.IsNullOrEmpty(SMTPCred.SMTP_Password) AndAlso
                ...
                rb = Error_MailHandling.SendSystemEmailUniversal(Error_Helper.RegistryHiveValue,
                    Error_Helper.RegistryPath,
                    NachrichtHeader,
                    ErrorMessage1,
                    "",
                    New List(Of String),
