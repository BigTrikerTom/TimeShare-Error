...
''' </remarks>
Public Class Error_FileSearch
    Private Shared FolderSize As Long = 0
    Private Shared ReadOnly ErrorLog As New System.Text.StringBuilder
    Private Shared CRC32Table() As Integer
    Private Shared AbortProgress As Boolean
    Private Shared mRaiseErrors As Boolean
...
#Region "Events"
    ...
    Public Shared  Event MatchFound(ByVal sender As Object, ByVal FileName As String)
    ...
    Public Shared  Event RaiseError(ByVal sender As Object, ByVal ErrorMessage As String)

    ''' <summary>
    ''' Wird aufgerufen wenn der Suchvorgang vollständig ist
    ''' </summary>
    ''' <remarks></remarks>
    Public Shared Event SearchComplete(ByVal sender As Object, ByVal e As EventArgs)
#End Region
...
#Region "[Search] = SUCHFUNKTION -> Dateisuche mit Filter"
    ...
    Public Sub Search(ByVal Root As String, ByVal SearchWord As String, ByVal Filter As SearchFilter, ByRef FileArray As List(Of String))
        ...
        AbortProgress = False
        RaiseEvent SearchComplete(nothing, Nothing)
    End Sub

    Private Shared Sub DoSearch(ByVal Root As String, ByVal SearchWord As String, ByVal Filter As SearchFilter, ByRef FileArray As List(Of String))
        Try
            ...
            If Filter.Listing = SearchFilter.LO.BOTH Or Filter.Listing = SearchFilter.LO.FILES_ONLY Then
                For i As Integer = 0 To Files.Length
                    If FileFilter(Files(i).ToString, SearchWord, Filter) = True Then
                        ...
                        RaiseEvent MatchFound(nothing, Files(i))
                    End If
                Next
            End If

            For i As Integer = 0 To Folders.Length
                ...
                If Filter.Listing = SearchFilter.LO.BOTH Or Filter.Listing = SearchFilter.LO.FOLDERS_ONLY Then
                    If Recurse = True Then
                        ...
                        RaiseEvent MatchFound(Nothing, Folders(i))
                    End If
                End If
        ...
        Catch Ex As Exception
            ...
            If mRaiseErrors = True Then
                ErrorLog.Append(Ex.Message & vbCrLf)
                RaiseEvent RaiseError(Nothing, Ex.Message)
            End If
        End Try
    End Sub
#End Region
...
#Region "[ListAll] = AUFLISTUNGSFUNKTION -> Auflistung von Dateien und Ordnern"
    ...
    Public shared Sub ListAll(ByVal Root As String, ByRef FileArray As List(Of String), Optional ByVal FFBFilter As SearchFilter.LO = SearchFilter.LO.BOTH)
        ErrorLog.Remove(0, ErrorLog.Length)
        AbortProgress = False
        ...
        RaiseEvent SearchComplete(Nothing, Nothing)
    End Sub
    Private Shared Sub DoListAll(ByVal Root As String, ByRef FileArray As List(Of String), Optional ByVal ListingOption As SearchFilter.LO = SearchFilter.LO.BOTH)
        Try
            If AbortProgress = True Then Exit Sub
            Dim Files() As String = System.IO.Directory.GetFiles(Root)
            Dim Folders() As String = System.IO.Directory.GetDirectories(Root)
...
#Region "[GetFolderSize] = Ordnergröße ermitteln"
    ...
    Public Shared Function GetFolderSize(ByVal Root As String, ByVal SizeFormat As SearchFilter.ST) As Long
        Dim TmpSize As Long = 0
        Dim FolderSizeLoc As Double = 0
        ErrorLog.Remove(0, ErrorLog.Length)
        DoGetFolderSize(Root, SizeFormat)

        Select Case SizeFormat
            Case SearchFilter.ST.BYTES : FolderSizeLoc /= 1
            Case SearchFilter.ST.KILO_BYTES : FolderSizeLoc /= 1024
            Case SearchFilter.ST.MEGA_BYTES : FolderSizeLoc /= 1048576
            Case SearchFilter.ST.GIGA_BYTES : FolderSizeLoc /= 1073741824
        End Select

        Return CLng(FolderSizeLoc)
    End Function
    Private Shared Sub DoGetFolderSize(ByVal Root As String, ByVal SizeFormat As SearchFilter.ST)
        Try
            Dim Files() As String = System.IO.Directory.GetFiles(Root)
            Dim Folders() As String = System.IO.Directory.GetDirectories(Root)
...
#End Region
    ...
    ''' </summary>
    Public Shared Function GetErrorLog() As String
    ...
    Public Shared Function GetCRC32(ByVal FileName As String) As String
        ...
        Loop
        Return System.Convert.ToString(Not (CRC32Result), 16)
    End Function

    ''' <summary>
    ''' Diese Funktion berechnet den MD5 Hash einer Datei
    ''' </summary>
    Public Shared Function GetMD5(ByVal File As String) As String
        ...
        For i As Integer = 0 To HashValue.Length - 1
            Tmp = System.Convert.ToString(HashValue(i), 16)
    ...
    ''' </summary>
    Public Shared Function GetSHA(ByVal File As String) As String
        Dim FN As New FileStream(File, FileMode.Open, FileAccess.Read, FileShare.Read, 8192)
        Dim HashValue(0) As Byte
        Dim Result As String = ""
        Dim Tmp As String = ""
...
#Region "Private Methods"
    Private Shared Function FileFilter(ByVal FileName As String, ByVal SearchWord As String, ByVal Filter As SearchFilter) As Boolean
        'Fastest First Slowest Last!
        Dim IsFileValid As Boolean = True
        Dim Extensions() As String
        Dim FileArray() As String

        If AbortProgress = True Then
            Return IsFileValid
        End If
        ...
        If SearchWord <> "" Then
            If FileName.Contains(SearchWord) Then
                IsFileValid = True
            End If
        End If
        ...
        If IsFileValid Then
            If Filter.ExcludeFiles <> "" Then
                FileArray = Filter.ExcludeFiles.Split(CChar(","))
                For i As Integer = 0 To FileArray.Length
        ...
        If IsFileValid Then
            If Filter.ExcludeFileTypes <> "" Then
                Extensions = Filter.ExcludeFileTypes.Split(CChar(","))
                For i As Integer = 0 To Extensions.Length
                    If Extensions(i) = Path.GetExtension(FileName.ToLower.Replace(".", "")) Then
                        IsFileValid = False
                        Exit For
                    End If
                Next
        ...
        If IsFileValid Then
            If Filter.FileTypes <> "" Then
                IsFileValid = False
                Extensions = Filter.FileTypes.Split(CChar(","))
                For i As Integer = 0 To Extensions.Length
                    If Extensions(i) = Path.GetExtension(FileName.ToLower.Replace(".", "")) Then
                        IsFileValid = True
                        Exit For
            ...
            If Filter.MaxSize > 0 Then
                Select Case Filter.SizeType
                    Case SearchFilter.ST.BYTES
                        If New FileInfo(FileName).Length > Filter.MaxSize Then IsFileValid = False
                    Case SearchFilter.ST.KILO_BYTES
                        If New FileInfo(FileName).Length \ 1024 > Filter.MaxSize Then IsFileValid = False
                    Case SearchFilter.ST.MEGA_BYTES
                        If New FileInfo(FileName).Length \ 1048576 > Filter.MaxSize Then IsFileValid = False
                    Case SearchFilter.ST.GIGA_BYTES
                        If New FileInfo(FileName).Length \ 1073741824 > Filter.MaxSize Then IsFileValid = False
                End Select
            End If
        End If
        ...
        If IsFileValid Then
            If Filter.MinSize > 0 Then
                Select Case Filter.SizeType
                    Case SearchFilter.ST.BYTES
                        If New FileInfo(FileName).Length < Filter.MinSize Then IsFileValid = False
                    Case SearchFilter.ST.KILO_BYTES
                        If New FileInfo(FileName).Length \ 1024 < Filter.MinSize Then IsFileValid = False
                    Case SearchFilter.ST.MEGA_BYTES
                        If New FileInfo(FileName).Length \ 1048576 < Filter.MinSize Then IsFileValid = False
                    Case SearchFilter.ST.GIGA_BYTES
                        If New FileInfo(FileName).Length \ 1073741824 < Filter.MinSize Then IsFileValid = False
                End Select
            End If
        End If
            ...
            If Filter.MaxDate <> Nothing Then
                Select Case Filter.DateType
                    Case SearchFilter.DT.CREATE_TIME
                        If File.GetCreationTime(FileName) > Filter.MaxDate Then
                            IsFileValid = False
                        End If
                    Case SearchFilter.DT.LAST_ACCESS_TIME
                        If File.GetLastAccessTime(FileName) > Filter.MaxDate Then
                            IsFileValid = False
                        End If
                    Case SearchFilter.DT.LAST_WRITE_TIME
                        If File.GetLastWriteTime(FileName) > Filter.MaxDate Then
                            IsFileValid = False
                        End If
                End Select
            End If
        ...
        If IsFileValid Then
            If Filter.MinDate <> Nothing Then
                Select Case Filter.DateType
                    Case SearchFilter.DT.CREATE_TIME
                        If File.GetCreationTime(FileName) < Filter.MinDate Then
                            IsFileValid = False
                        End If
                    Case SearchFilter.DT.LAST_ACCESS_TIME
                        If File.GetLastAccessTime(FileName) < Filter.MinDate Then
                            IsFileValid = False
                        End If
                    Case SearchFilter.DT.LAST_WRITE_TIME
                        If File.GetLastWriteTime(FileName) < Filter.MinDate Then
                            IsFileValid = False
                        End If
                End Select
            End If
        ...
        If IsFileValid Then
            If Filter.MD5Hash <> "" Then
                If Filter.MD5Hash.ToUpper <> GetMD5(FileName.ToUpper) Then
                    IsFileValid = False
                End If
            End If
        End If
        ...
        If IsFileValid Then
            If Filter.SHAHash <> "" Then
                If Filter.SHAHash.ToUpper <> GetSHA(FileName.ToUpper) Then
                    IsFileValid = False
                End If
            End If
        End If
        ...
        If IsFileValid Then
            If Filter.CRCHash <> "" Then
                If Filter.CRCHash.ToUpper <> GetCRC32(FileName.ToUpper) Then IsFileValid = False
            End If
        End If
    ...
    Private Shared Function AllowedFolder(ByVal CurrentFolder As String, ByVal ExcludeFolders As String) As Boolean
        Dim ExcludeArr() As String
        AllowedFolder = True
        If ExcludeFolders = "" Then Exit Function
        ExcludeArr = ExcludeFolders.Split(CChar(","))

        For i As Integer = 0 To ExcludeArr.Length
            If ExcludeArr(i).Contains("\") Then
                'Wenn ein Ordner mit Pfad angegeben wurde
                If CurrentFolder.Contains(ExcludeArr(i)) Then
                    AllowedFolder = False
                    Exit For
                End If
            Else
    ...
    Private Shared Sub CreateCRC32Table()
        ...
        For i As Integer = 0 To 255
            ...
            For j As Integer = 8 To 1 Step -1
                If CBool((DWCRC And 1)) Then
                    DWCRC = Error_VarConvert.ConvertToInteger(((DWCRC And &HFFFFFFFE) \ 2&) And &H7FFFFFFF, 0) 
    ...
    Private Shared Function GetFileExt(ByVal File As String) As String
